...
private enum NativeArtworkManagementMode {
    case upload
    case manage
}
...
private struct NativeArtworkManagementView: View {
    @EnvironmentObject private var language: LanguageManager
    @EnvironmentObject private var auth: AuthManager
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 14) {
                ...
                sectionCard {
                    VStack(alignment: .leading, spacing: 12) {
                        Text(fieldLabelImages)
                            .font(.headline)
                        Text(imageHelperText)
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                        ...
                        if imageCount == 0 {
                            ...
                            Text(language.language == .chinese ? "点按缩略图设置封面图。" : "Tap a thumbnail to set the cover image.")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                    }
